    ...
    <div style={{
      ...
      <div style={{ padding: '14px 14px 10px' }}>
        <div style={{
          ...
          border: `1px solid ${isAdmin ? 'rgba(191,90,242,0.22)' : 'rgba(255,255,255,0.08)'}`,
          borderRadius: 12,
        }}>
          <div style={{
            width: 32, height: 32, borderRadius: '50%', flexShrink: 0,
            background: avatarBg + '28', border: `2px solid ${avatarBg}55`,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: '0.75rem', fontWeight: 800, color: avatarBg,
          }}>{initial}</div>
        ...
        {/* Non-admin: SCT balance + plan badge */}
        {!isAdmin && (
          <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
            <div style={{
              flex: 1, display: 'flex', alignItems: 'center', gap: 5,
              padding: '6px 10px', background: 'rgba(255,159,10,0.08)',
              border: '1px solid rgba(255,159,10,0.15)', borderRadius: 8,
            }}>
              <Coins size={12} color="#ff9f0a"/>
              <span style={{ fontSize: '0.73rem', fontWeight: 700, color: '#ff9f0a' }}>
                {user?.sctBalance ?? '—'} SCT
              </span>
            </div>
            {!isProvider && (
              <div onClick={() => navigate('/app/plans')}
                style={{
                  display: 'flex', alignItems: 'center', gap: 4,
                  padding: '6px 10px', background: `${planColor}10`,
                  border: `1px solid ${planColor}30`, borderRadius: 8, cursor: 'pointer',
                }} title="Manage plan">
        ...
        {/* Admin: full-access badge */}
        {isAdmin && (
          <div style={{
            marginTop: 8, padding: '7px 12px', textAlign: 'center',
            background: 'rgba(191,90,242,0.1)', border: '1px solid rgba(191,90,242,0.25)', borderRadius: 8,
          }}>
            <span style={{ fontSize: '0.72rem', fontWeight: 700, color: '#bf5af2', letterSpacing: '0.06em' }}>
              ⚡ Full System Access
            </span>
          </div>
        )}
      ...
      <div style={{ height: 1, background: 'rgba(255,255,255,0.06)', margin: '0 14px 6px' }}/>
